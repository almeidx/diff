...
<svelte:head>
	<title>{packageName} {fromVersion}...{toVersion} - Diff</title>
</svelte:head>
	...
	<header class="flex items-center justify-between px-4 py-3 bg-bg-secondary border-b border-border sticky top-0 z-[100]">
		<div class="flex items-center gap-4">
			...
			<span class="flex items-center gap-2 text-sm">
				<span class="px-2 py-0.5 bg-bg-tertiary rounded text-text-secondary text-xs font-medium">{packageLabel}</span>
				<span class="text-text-muted">/</span>
				<span class="font-medium font-mono">{packageName}</span>
			</span>
		</div>
	...
	<div class="flex items-end gap-4 p-4 bg-bg-primary border-b border-border max-md:flex-col max-md:items-stretch">
		<div class="min-w-[200px] flex-1 max-w-xs max-md:min-w-0 max-md:max-w-none">
			<VersionSelector
				{versions}
				value={fromVersion}
				onchange={(v) => onNavigate(v, toVersion)}
				label="From"
				id="from-version"
		...
		<div class="min-w-[200px] flex-1 max-w-xs max-md:min-w-0 max-md:max-w-none">
			<VersionSelector
				{versions}
				value={toVersion}
				onchange={(v) => onNavigate(fromVersion, v)}
				label="To"
				id="to-version"
			...
			<div class="max-w-[600px] p-6 bg-bg-secondary border border-border rounded-lg">
				{#if error.type === 'invalid_version'}
					...
					<div class="flex flex-wrap gap-2">
						{#each error.availableVersions.slice(0, 20) as version}
							<span class="px-2 py-1 bg-bg-tertiary rounded text-xs font-mono">{version}</span>
						{/each}
						{#if error.availableVersions.length > 20}
