...
<div class="flex flex-col min-h-screen">
	...
	{:else if data.diff}
		<div class="flex items-center justify-between px-4 py-3 bg-bg-primary border-b border-border max-md:flex-wrap max-md:gap-2">
			...
			<div class="flex items-center gap-3">
				{#await data.compareUrl then compareUrl}
					{#if compareUrl}
						<a href={compareUrl} class="flex items-center justify-center w-8 h-8 rounded-md text-text-secondary transition-all hover:bg-bg-secondary hover:text-text-primary hover:no-underline" target="_blank" rel="noopener" aria-label="Compare on GitHub" title="Compare on GitHub">
